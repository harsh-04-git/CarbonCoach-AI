...
export default function App() {
  ...
  // Helper selectors
  const prioritizedActions: RankedAction[] = auditInput
    ? getPrioritizedActions(auditInput)
    : [];

  return (
    <div
      className="min-h-screen bg-[#F0FDF4] text-slate-800 flex flex-col font-sans px-4 py-8"
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-emerald-700 font-mono font-extrabold tracking-widest uppercase mt-0.5">
              {getSubHeaderMessage(activeState) ||
                "State 0: Dynamic Profile Selection"}
            </p>
          </div>
        </div>
      ...
      <main className="flex-grow w-full">
        {activeState === 0 && (
          <Onboarding onSelectPersona={handleSelectPersona} />
        )}
        ...
        {profile && auditInput && activeState >= 2 && (
          <div className="space-y-6">
            {/* Nav Switch Tabs */}
            <div
              className="max-w-4xl mx-auto flex flex-wrap gap-2 p-1.5 bg-white/90 rounded-2xl border border-emerald-100 backdrop-blur shadow-sm"
              id="floating-navigation-rail"
            >
              {[
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      localStorage.setItem(
                        STORAGE_KEYS.LEGACY_ACTIVE_STATE,
                        tab.state.toString()
                      );
                    }}
      ...
      <footer className="max-w-4xl mx-auto w-full text-center py-8 mt-12 border-t border-slate-100 text-[10px] text-slate-400 font-mono tracking-wider shrink-0 uppercase flex flex-col gap-2">
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <span className="flex items-center gap-1.5">
            <Leaf className="w-3 h-3" /> Net Zero Target
          </span>
        </div>
      </footer>
    </div>
