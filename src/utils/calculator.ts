 ...
 */
export { getFinancialSavings } from "./financialSavings";
 ...
 * - Transport: Vehicle-specific factors (e.g. 0.35kg/mile for car, 0.1kg/mile for bus)
 * - Energy: Average monthly bill scaled, plus AC usage at 1.2kg CO2 per hour (Indian grid average)
 * - Food: Baseline diet habits (Vegetarian: 1500kg, Mixed: 2300kg, Non-vegetarian: 3300kg/year)
 * - Shopping: Deliveries frequency (Rarely: 100kg, Medium: 350kg, High: 800kg/year)
 * - Flights: Average flight footprint of 500kg CO2 per flight
 * Score is clamped to [10, 99].
 *
 * @param input - The lifestyle audit inputs from the user
 * @returns The calculated annual emissions breakdown and score profile
 */
export function calculateEmissions(input: CarbonAuditInput): CarbonProfile {
  ...
  const transportFactor =
    researchData.emission_factors.transport[input.transport] || 0;
  ...
  const electricityKg =
    annualKwh * researchData.emission_factors.energy.electricity_kwh_kg;

  // AC usage: hours/day * 365 days * ac_hour_kg
  const acKg =
    input.ac_usage * 365 * researchData.emission_factors.energy.ac_hour_kg;
  ...
  const shoppingKg =
    researchData.emission_factors.shopping[input.shopping_frequency] || 350;

  // 5. Flights CO2 (flights per year * standard flight emissions)
  const flightsKg =
    input.flights_per_year * researchData.emission_factors.flights.average_kg;
  ...
  const totalEmissions =
    Math.round(
      (transportTons + energyTons + foodTons + shoppingTons + flightsTons) * 100
    ) / 100;
  ...
  const carbonScore = Math.max(
    10,
    Math.min(99, Math.round(100 - totalEmissions * 7.5))
  );
