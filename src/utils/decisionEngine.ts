...
/**
 * Represents a reductive action tailored and ranked for a user's carbon footprint.
 */
export interface RankedAction extends ReductiveAction {
  /** The calculated annual carbon savings in kg adjusted for user inputs */
  customSavingKg: number;
  /** The ranking score calculated as (customSavingKg / 50) * ease */
  priorityScore: number;
}

/**
 * Tailors standard reductive action saving potentials using the user's specific lifestyle inputs,
 * filters out irrelevant or zero-impact actions, and sorts them descending by their priority score.
 *
 * @param input - The lifestyle audit inputs from the user
 * @returns A sorted array of ranked reductive actions
 */
export function getPrioritizedActions(input: CarbonAuditInput): RankedAction[] {
  return researchData.reductive_actions
    .map((action) => {
      ...
      switch (action.id) {
        case "reduce_ac":
          if (input.ac_usage === 0) {
            ...
            customSavingKg = Math.round(
              action.annual_saving_kg * (input.ac_usage / 4)
            );
          }
          break;
        ...
        case "public_transit_2x":
          if (input.transport !== "car") {
            ...
            customSavingKg = Math.round(
              action.annual_saving_kg * (input.commute_distance / 40)
            );
          }
          break;
