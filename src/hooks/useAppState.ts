...
/**
 * Custom hook to manage the global state of the CarbonCoach application.
 * Handles loading/saving of user inputs, profile calculations, and navigation state from localStorage.
 * Maintains compatibility with legacy activeState key.
 */
export function useAppState() {
  ...
  useEffect(() => {
    try {
      ...
      let savedState = localStorage.getItem(STORAGE_KEYS.ACTIVE_STATE);
      if (!savedState) {
        savedState = localStorage.getItem(STORAGE_KEYS.LEGACY_ACTIVE_STATE);
      }
  ...
  const saveAuditInput = (input: CarbonAuditInput) => {
    ...
    localStorage.setItem(STORAGE_KEYS.LEGACY_ACTIVE_STATE, "2");
  ...
  const handleSelectPersona = (key: PersonaKey) => {
    ...
    if (key === "student_commuter") {
      initialInput = researchData.persona_presets.student_commuter;
    } else if (key === "working_professional") {
      initialInput = researchData.persona_presets.working_professional;
    } else if (key === "family_household") {
      initialInput = researchData.persona_presets.family_household;
    } else {
      initialInput = {
        transport: "car",
        commute_distance: 0,
    ...
    localStorage.setItem(STORAGE_KEYS.LEGACY_ACTIVE_STATE, "1");
  };
