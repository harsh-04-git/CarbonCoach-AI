...
export const CarbonAudit: React.FC<CarbonAuditProps> = ({
  ...
  return (
    <div
      className="max-w-2xl mx-auto bg-white rounded-3xl border border-emerald-100 shadow-lg p-6 md:p-8 animate-fade-in"
      id="carbon-audit-root"
    >
      {/* Back to selection */}
      <button
        onClick={onBack}
      ...
      <p className="text-sm text-slate-500 mb-6 font-medium">
        Fill out the 3 sections to establish your lifestyle benchmark emissions.
        You can also quickly toggle presets below.
      </p>
      ...
      <div className="flex justify-between items-center mb-8">
        {steps.map((label, idx) => (
          <React.Fragment key={idx}>
            ...
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-mono font-bold border transition ${
                  idx === activeStep
              ...
              <span
                className={`text-[11px] md:text-xs transition ${
                  idx === activeStep
            ...
            {idx < steps.length - 1 && (
              <div
                className={`flex-grow h-0.5 mx-2 md:mx-4 ${idx < activeStep ? "bg-emerald-300" : "bg-slate-200"}`}
              />
            )}
          </React.Fragment>
        ))}
          ...
          <div className="space-y-6 animate-fade-in" id="step-1-content">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  },
                  {
                    ...
                    label: "Train/Metro",
                    icon: Train,
                    color: "hover:border-teal-400 hover:text-teal-500",
                  },
                  {
                    value: "car",
                    label: "Personal Car",
                    icon: Car,
                    color: "hover:border-rose-400 hover:text-rose-500",
                  },
                ].map((item) => {
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setCommuteDistance(parseInt(e.target.value) || 0)
                }
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="text-sm font-bold text-slate-800 font-display flex items-center gap-1.5">
                  <Plane className="w-4 h-4 text-emerald-500" /> Flights taken
                  annually
                </label>
                <div className="text-emerald-700 font-mono font-bold text-sm bg-emerald-50 px-2.5 py-0.5 rounded-md border border-emerald-100">
              ...
              <input
                ...
                onChange={(e) =>
                  setFlightsPerYear(parseInt(e.target.value) || 0)
                }
        ...
        {activeStep === 1 && (
          <div className="space-y-6 animate-fade-in" id="step-2-content">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="text-sm font-bold text-slate-800 font-display flex items-center gap-1.5">
                  <Zap className="w-4 h-4 text-emerald-500" /> Monthly electric
                  bill
                </label>
                <div className="text-emerald-700 font-mono font-bold text-sm bg-emerald-50 px-2.5 py-0.5 rounded-md border border-emerald-100">
                  ₹{electricityBill} /mo
              ...
              <input
                ...
                onChange={(e) =>
                  setElectricityBill(parseInt(e.target.value) || 0)
                }
                className="w-full accent-emerald-500 bg-slate-100 h-1.5 rounded-lg cursor-pointer"
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 mt-2 font-sans flex items-center gap-1.5 font-medium">
                ...
                AC uses significant compressor loads. Excellent reduction
                target.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6 animate-fade-in" id="step-3-content">
                ...
                {[
                  {
                    value: "vegetarian",
                    label: "Vegetarian",
                    desc: "No meat, mostly plant-based",
                  },
                  {
                    value: "mixed",
                    label: "Mixed Diet",
                    desc: "Balanced dairy, fish, poultry",
                  },
                  {
                    value: "non-vegetarian",
                    label: "Heavy Meat",
                    desc: "Heavy beef or red meats",
                  },
                ].map((item) => (
                  <button
                    key={item.value}
                  ...
                  >
                    <span className="text-sm font-bold font-display mb-1">
                      {item.label}
                    </span>
                    <span className="text-[11px] text-slate-500 font-semibold font-sans leading-tight">
                      {item.desc}
                    </span>
                  </button>
                ))}
            ...
            <div>
              <label className="block text-sm font-bold text-slate-800 font-display mb-3 flex items-center gap-1.5">
                <ShoppingBag className="w-4 h-4 text-emerald-500" /> Online
                ...
                {[
                  {
                    value: "rarely",
                    label: "Rarely / Low",
                    desc: "Combine orders once a month",
                  },
                  {
                    value: "medium",
                    label: "Medium",
                    desc: "Weekly single drop deliveries",
                  },
                  {
                    value: "high",
                    label: "High frequency",
                    desc: "Multiple express drops weekly",
                  },
                ].map((item) => (
                  <button
                    key={item.value}
                    type="button"
                    onClick={() =>
                      setShoppingFrequency(item.value as ShoppingFrequency)
                    }
                    className={`flex flex-col text-left p-4 rounded-xl border transition cursor-pointer min-h-[48px] ${
