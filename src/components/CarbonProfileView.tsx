...
import { Leaf, Zap, ShoppingCart, Car, Plane } from "lucide-react";
import { getScoreColor, getScoreRating, getCategoryDrivers } from "../utils/ui/formatters";
import {
  GLOBAL_BENCHMARKS,
  MAX_CHART_TONS,
...
export const CarbonProfileView: React.FC<CarbonProfileViewProps> = ({
  profile,
  onContinue,
  onRecalculate,
}) => {
  ...
  const strokeDashoffset =
    circumference - (profile.carbonScore / 100) * circumference;

  return (
    <div
      className="max-w-4xl mx-auto px-4 py-6 animate-fade-in"
    ...
    >
      <div className="grid md:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="md:col-span-5 bg-white rounded-3xl border border-emerald-100 p-6 flex flex-col justify-between shadow-sm">
          ...
          <div className="mt-4 p-4 bg-emerald-50/40 border border-emerald-100 rounded-2xl text-center">
            <div
              className={`text-sm font-black font-display ${getScoreColor(profile.carbonScore)}`}
            >
              {rating.label}
            </div>
              ...
              {[
                {
                  ...
                  percent: profile.breakdownPercentages.transport,
                  icon: Car,
                  color: "bg-amber-500",
                },
                {
                  ...
                  percent: profile.breakdownPercentages.energy,
                  icon: Zap,
                  color: "bg-teal-500",
                },
                {
                  ...
                  percent: profile.breakdownPercentages.food,
                  icon: Leaf,
                ...
                },
                {
                  ...
                  percent: profile.breakdownPercentages.shopping,
                  icon: ShoppingCart,
                  color: "bg-purple-500",
                },
                {
                  ...
                  percent: profile.breakdownPercentages.flights,
                  icon: Plane,
                  color: "bg-sky-500",
                },
              ].map((cat, i) => {
                ...
                return (
                  <div
                    ...
                    id={`category-bar-${cat.name.toLowerCase().replace(/ /g, "-")}`}
                  >
      ...
      <div
        className="bg-white rounded-3xl border border-emerald-100 p-6 md:p-8 shadow-sm mt-6 animate-fade-in"
        id="carbon-relative-benchmarks"
      >
        ...
        <p className="text-xs text-slate-500 font-sans font-medium mb-5 leading-relaxed">
          How does your footprint stack up relative to climate thresholds and
          average citizens around the world? The Paris Agreement targets a
          sustainable threshold of{" "}
          <strong>
            under {GLOBAL_BENCHMARKS.PARIS_ACCORD.value.toFixed(1)} Metric Tons
            CO₂
          </strong>{" "}
          per capita by 2030.
        </p>
          ...
          <div className="relative w-full h-3 bg-slate-200 rounded-full">
            ...
            <div className="absolute left-0 right-0 h-full flex justify-between pointer-events-none">
              <div className="w-[1px] h-5 bg-emerald-500/60 relative -top-1">
                <span className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-[9px] font-mono font-bold text-emerald-600">
                  0.0T
                </span>
              </div>
              <div
                className="w-[1px] h-5 bg-teal-500/60 relative -top-1"
                style={{
                  left: `${(GLOBAL_BENCHMARKS.INDIA_AVG.value / MAX_CHART_TONS) * 100}%`,
                }}
              >
                <span className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-[9px] font-mono font-bold text-teal-650">
                  {GLOBAL_BENCHMARKS.INDIA_AVG.value}T (India)
                </span>
              </div>
              <div
                className="w-[1px] h-5 bg-amber-500/60 relative -top-1"
                style={{
                  left: `${(GLOBAL_BENCHMARKS.GLOBAL_AVG.value / MAX_CHART_TONS) * 100}%`,
                }}
              >
                <span className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-[9px] font-mono font-bold text-amber-600">
                  {GLOBAL_BENCHMARKS.GLOBAL_AVG.value}T (Global)
                </span>
              </div>
              <div
                className="w-[1px] h-5 bg-rose-500/60 relative -top-1"
                style={{ left: "100%" }}
              >
                <span className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-[9px] font-mono font-bold text-rose-600">
                  {GLOBAL_BENCHMARKS.USA_AVG.value}T (USA)
                </span>
              </div>
            </div>
            ...
            {(() => {
              ...
              const userPct = Math.min(
                100,
                Math.max(0, (profile.annualEmissions / maxTons) * 100)
              );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mt-8">
          <div className="p-4 bg-emerald-50/50 border border-emerald-100 rounded-2xl flex flex-col justify-between">
            <span className="text-[10px] font-mono font-bold text-emerald-600 tracking-wider block uppercase mb-1">
              {GLOBAL_BENCHMARKS.PARIS_ACCORD.label}
            </span>
            <div className="text-base font-extrabold text-slate-900 font-display">
              &lt; {GLOBAL_BENCHMARKS.PARIS_ACCORD.value.toFixed(1)} Tons
            </div>
            <p className="text-[10px] text-slate-400 mt-2 font-medium">
              {GLOBAL_BENCHMARKS.PARIS_ACCORD.desc}
            </p>
          </div>
          <div className="p-4 bg-teal-50/50 border border-teal-100 rounded-2xl flex flex-col justify-between">
            <span className="text-[10px] font-mono font-bold text-teal-600 tracking-wider block uppercase mb-1">
              {GLOBAL_BENCHMARKS.INDIA_AVG.label}
            </span>
            <div className="text-base font-extrabold text-slate-900 font-display">
              {GLOBAL_BENCHMARKS.INDIA_AVG.value.toFixed(1)} Tons
            </div>
            <p className="text-[10px] text-slate-400 mt-2 font-medium">
              {GLOBAL_BENCHMARKS.INDIA_AVG.desc}
            </p>
          </div>
          <div className="p-4 bg-amber-50/50 border border-amber-100 rounded-2xl flex flex-col justify-between">
            <span className="text-[10px] font-mono font-bold text-amber-600 tracking-wider block uppercase mb-1">
              {GLOBAL_BENCHMARKS.GLOBAL_AVG.label}
            </span>
            <div className="text-base font-extrabold text-slate-900 font-display">
              {GLOBAL_BENCHMARKS.GLOBAL_AVG.value.toFixed(1)} Tons
            </div>
            <p className="text-[10px] text-slate-400 mt-2 font-medium">
              {GLOBAL_BENCHMARKS.GLOBAL_AVG.desc}
            </p>
          </div>
          <div className="p-4 bg-rose-50/50 border border-rose-100 rounded-2xl flex flex-col justify-between">
            <span className="text-[10px] font-mono font-bold text-rose-600 tracking-wider block uppercase mb-1">
              {GLOBAL_BENCHMARKS.USA_AVG.label}
            </span>
            <div className="text-base font-extrabold text-slate-900 font-display">
              {GLOBAL_BENCHMARKS.USA_AVG.value.toFixed(1)} Tons
            </div>
            <p className="text-[10px] text-slate-400 mt-2 font-medium">
              {GLOBAL_BENCHMARKS.USA_AVG.desc}
            </p>
          </div>
        </div>
      </div>
      ...
      <div
        className="bg-white rounded-3xl border border-emerald-100 p-6 md:p-8 shadow-sm mt-6 animate-fade-in"
        id="score-explanation-card"
      >
        <h3 className="text-md font-black font-display text-slate-900 mb-2 flex items-center gap-2">
          <span>📊</span> Carbon Score Calibration Breakdown
        </h3>
        <p className="text-xs text-slate-500 font-sans font-medium mb-5 leading-relaxed">
          Your score starts at a perfect baseline score of{" "}
          <strong>100 points</strong> and is reduced according to the specific
          carbon penalties of your category footprints (scaled as{" "}
          <strong>-7.5 score deduction points per metric ton CO₂</strong>). Here
          is exactly why you scored <strong>{profile.carbonScore}</strong>/100:
        </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
          {[
            {
              n: "Daily Transport",
              pts: Math.round(profile.breakdown.transport * 7.5 * 10) / 10,
              t: profile.breakdown.transport,
            },
            {
              n: "Home & Energy",
              pts: Math.round(profile.breakdown.energy * 7.5 * 10) / 10,
              t: profile.breakdown.energy,
            },
            {
              n: "Food & Diet",
              pts: Math.round(profile.breakdown.food * 7.5 * 10) / 10,
              t: profile.breakdown.food,
            },
            {
              n: "Online Shopping",
              pts: Math.round(profile.breakdown.shopping * 7.5 * 10) / 10,
              t: profile.breakdown.shopping,
            },
            {
              n: "Aviation Flights",
              pts: Math.round(profile.breakdown.flights * 7.5 * 10) / 10,
              t: profile.breakdown.flights,
            },
          ].map((item, idx) => (
            <div
              key={idx}
              className="p-3 bg-slate-50 border border-slate-150 rounded-2xl text-center"
            >
              ...
              <span className="text-[10px] font-sans text-slate-400 block font-medium">
                ({item.t} Tons)
              </span>
            </div>
          ))}
          ...
          return (
            <div className="grid sm:grid-cols-2 gap-4 pt-4 border-t border-slate-100 text-xs">
              <div className="p-3.5 bg-rose-50/40 border border-rose-100 rounded-2xl">
                <span className="text-[10px] font-mono font-black uppercase text-rose-700 block mb-1">
                  🔥 Biggest Driver
                </span>
                <p className="font-sans font-semibold text-slate-800">
                  {biggest.name} ({biggest.tons} Tons / -
                  {Math.round(biggest.tons * 7.5 * 10) / 10} points penalty) is
                  your leading footprint source, coming from {biggest.desc}.
                </p>
              </div>
              <div className="p-3.5 bg-emerald-50/40 border border-emerald-100 rounded-2xl">
                <span className="text-[10px] font-mono font-black uppercase text-emerald-700 block mb-1">
                  🌱 Lowest Driver (Best Performing)
                </span>
                <p className="font-sans font-semibold text-slate-800">
                  {lowest.name} ({lowest.tons} Tons / -
                  {Math.round(lowest.tons * 7.5 * 10) / 10} points penalty) is
                  your lowest-emission Category, coming from {lowest.desc}.
                </p>
              </div>
            </div>
          );
