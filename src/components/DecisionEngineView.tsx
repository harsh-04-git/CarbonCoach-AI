...
import { getCategoryIcon, getCategoryColor } from "../utils/ui/formatters";
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 bg-emerald-50 border border-emerald-100 p-4 rounded-xl shadow-sm shadow-emerald-50/50">
        <div>
          <h3 className="text-sm font-bold text-emerald-800 font-display flex items-center gap-1.5 mb-1">
            <Flame className="w-4 h-4 text-emerald-600" /> Customized Logical
            Prioritization Algorithm
          </h3>
          <p className="text-xs text-slate-600 font-sans max-w-xl font-medium leading-relaxed">
            This roadmap maps emission savings against ease metrics{" "}
            <span className="text-slate-900 font-bold font-mono">
              (Saving × Ease Rating)
            </span>
            . Swapping red meat when already vegetarian has 0kg impact, so those
        ...
        {actions.map((action, idx) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className="flex items-center gap-3.5 flex-grow pr-4">
                  ...
                  <div className="flex-grow">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <span
                        className={`inline-flex items-center gap-1 text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-md border ${getCategoryColor(action.category)}`}
                      >
                    ...
                    <h4
                      className={`text-sm md:text-md font-bold font-display leading-tight ${isCommitted ? "text-emerald-700" : "text-slate-950"}`}
                    >
                      {action.title}
                    </h4>
                  </div>
                ...
                <div className="flex items-center gap-4 text-right flex-shrink-0">
                  <div className="hidden sm:block">
                    <div className="text-[10px] text-slate-400 font-extrabold uppercase font-mono tracking-wider">
                      ANNUAL SAVINGS
                    </div>
              ...
              {isExpanded && (
                <div
                  className="px-4 pb-4 pt-1 border-t border-slate-100 text-xs md:text-sm text-slate-600 space-y-3 animate-fade-in"
                  id={`expanded-body-${action.id}`}
                >
                  ...
                  <div className="sm:hidden flex justify-between p-2 bg-slate-50 rounded-lg border border-slate-200 text-center">
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-mono font-extrabold">
                        CO₂ Saved
                      </span>
                      <span className="text-xs font-bold font-mono text-emerald-600">
                        -{action.customSavingKg} kg/yr
                      </span>
                    </div>
                    <div>
                      <span className="text-slate-400 block text-[10px] uppercase font-mono font-extrabold font-black">
                        Rupee Saved
                      </span>
                      <span className="text-xs font-extrabold font-mono text-teal-650">
                        ₹
                        {getFinancialSavings(
                          action.category,
                          action.customSavingKg
                  ...
                  {(() => {
                    let explanation =
                      "This general recommendation fits your category profile to help keep thresholds sustainable.";
                    ...
                    if (action.id === "reduce_ac") {
                      ...
                      if (
                        auditInput?.food_habits === "mixed" ||
                        auditInput?.food_habits === "non-vegetarian"
                      ) {
                      ...
                      if (
                        auditInput?.shopping_frequency === "high" ||
                        auditInput?.shopping_frequency === "medium"
                      ) {
                      ...
                      if (
                        auditInput?.electricity_bill &&
                        auditInput.electricity_bill > 0
                      ) {
                    ...
                    return (
                      <div className="p-3 bg-emerald-50/40 border border-emerald-100 rounded-xl flex items-start gap-2.5 text-xs">
                        ...
                        <div>
                          <span className="font-mono font-bold text-emerald-800 uppercase text-[9px] tracking-wider block mb-0.5">
                            Profile Explainability Link
                          </span>
                          <span className="font-sans text-slate-705 leading-relaxed font-semibold">
                            {explanation}
                          </span>
                        </div>
                      </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-3 pt-1">
                    ...
                    <div className="p-3 bg-slate-50 border border-slate-150 rounded-xl flex flex-col justify-between">
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-slate-450 font-mono font-extrabold text-[10px] uppercase">
                          Ease of changes:
                        </span>
