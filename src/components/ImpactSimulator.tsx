...
export const ImpactSimulator: React.FC<ImpactSimulatorProps> = ({
  ...
  const totalSavedKg = committedActions.reduce((sum, a) => sum + a.customSavingKg, 0);
  const totalSavedTons = Math.round((totalSavedKg / 1000) * 100) / 100;
  const totalSavedRs = committedActions.reduce((sum, a) => sum + getFinancialSavings(a.category, a.customSavingKg), 0);
  
  // Calculate projected
  const projectedEmissions = Math.max(0, Math.round((initialProfile.annualEmissions - totalSavedTons) * 100) / 100);
  const reductionPercentage = initialProfile.annualEmissions > 0 
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-4 animate-fade-in" id="impact-simulator-root">
        ...
        <div className="md:col-span-5 flex flex-col gap-4">
          ...
          <div className="bg-white rounded-3xl border border-emerald-100 p-5 shadow-sm select-none">
            ...
            <div className="space-y-4">
              <div>
                <span className="text-xs text-slate-400 block font-bold uppercase tracking-wide">Baseline Audit</span>
                <span className="text-xl font-bold font-mono text-slate-700">
                  {initialProfile.annualEmissions} <span className="text-xs">Tons/Yr</span>
                </span>
              </div>
              ...
              <div className="flex justify-between items-center bg-emerald-50 border border-emerald-100 rounded-2xl p-3.5 text-emerald-800">
                <div>
                  <span className="text-[10px] uppercase font-mono text-emerald-700 block font-black">Reduction Match</span>
                  <span className="text-2xl font-black font-mono">{reductionPercentage}%</span>
                </div>
                <div className="text-right">
                  <span className="text-[10px] uppercase font-mono text-emerald-700 block font-black">Tons Deducted</span>
                  <span className="text-md font-bold font-mono text-emerald-800">-{totalSavedTons} Tons/yr</span>
                </div>
              </div>

              <div className="flex justify-between items-center bg-teal-50 border border-teal-100 rounded-2xl p-3.5 text-teal-850">
                <div>
                  <span className="text-[10px] uppercase font-mono text-teal-700 block font-black">Estimated Wallet savings</span>
                  <span className="text-lg font-black font-mono text-teal-900">₹{totalSavedRs.toLocaleString("en-IN")}/yr</span>
                </div>
                <div className="text-right">
                  <span className="text-[10px] uppercase font-mono text-teal-700 block font-black">Finances</span>
                  <span className="text-xs font-semibold font-sans text-teal-800">High-Yield Save</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-emerald-100 p-5 shadow-sm flex items-center gap-4">
            ...
            <div>
              ...
              <h4 className="text-base font-black text-slate-900 font-display">
                {treesPlantedOffset > 0 ? `+${treesPlantedOffset} mature trees` : "0 trees offset yet"}
              </h4>
              <p className="text-xs text-slate-500 font-sans font-medium mt-1 leading-relaxed">
                Offset potential of committed choices over 1 year of daily adherence.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-7 bg-white rounded-3xl border border-emerald-100 p-6 flex flex-col justify-between shadow-sm">
          <div>
            ...
            <p className="text-xs text-slate-500 mb-6 font-sans font-medium leading-relaxed">
              Check actions below to run carbon scenarios and simulate how minor habit tweaks instantly build up metric savings.
            </p>
                  ...
                  >
                    ...
                    <div className="text-right">
                      <span className="text-xs text-emerald-700 font-black font-mono block">
                        -{action.customSavingKg} <span className="text-[10px] font-normal text-slate-400">kg/yr</span>
                      </span>
                      <span className="text-[11px] text-teal-650 font-black font-mono block">
                        ₹{getFinancialSavings(action.category, action.customSavingKg).toLocaleString("en-IN")}/yr
                      </span>
          ...
          <div className="mt-6 pt-4 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-3">
            <div className="text-xs text-slate-500 flex items-center gap-1.5 font-sans font-medium">
              <ShieldCheck className="w-4 h-4 text-emerald-600" /> Grounded Calculations Verified
            </div>
            <div className="text-xs text-slate-450 font-bold text-center">
              Select actions to populate your <span className="text-emerald-700 font-bold text-xs">7-Day Weekly Carbon Challenge</span>!
            </div>
          </div>
        </div>
      </div>
