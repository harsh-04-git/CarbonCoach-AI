...
import { Award, Flame, CheckCircle, Circle, Trophy } from "lucide-react";
...
export const WeeklyChallenge: React.FC = () => {
  ...
  const getChallengeFinancialSaving = (
    day: number,
    savingKg: number
  ): number => {
    switch (day) {
      case 1:
        return Math.round(savingKg * 10); // transport multiplier
      case 2:
        return Math.round(savingKg * 15); // energy multiplier
      case 3:
        return Math.round(savingKg * 8); // food multiplier
      case 4:
        return Math.round(savingKg * 15); // energy multiplier
      case 5:
        return Math.round(savingKg * 8); // food multiplier
      case 6:
        return Math.round(savingKg * 15); // energy multiplier
      default:
        return Math.round(savingKg * 15); // energy multiplier (LED audit etc.)
    }
  };
    ...
    >
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-orange-50/60 border border-orange-100 p-4 rounded-2xl flex items-center justify-between shadow-sm">
          <div>
            <span className="text-xs text-slate-400 font-bold font-mono block uppercase">
              Active streak
            </span>
        ...
        <div className="bg-white border border-emerald-100 p-4 rounded-2xl flex flex-col justify-between shadow-sm">
          <div className="flex justify-between items-center text-xs text-slate-400 font-bold mb-1.5">
            ...
            <span className="font-mono font-black text-emerald-600">
              {completedDays.length} / 7 Days Done
            </span>
          </div>
        ...
        <div className="bg-white border border-emerald-100 p-4 rounded-2xl flex items-center justify-between shadow-sm">
          <div>
            <span className="text-xs text-slate-400 font-mono block uppercase font-bold">
              Challenge Savings
            </span>
            <span className="text-lg font-black font-mono text-teal-600 block">
              -{totalSavedKg.toFixed(1)}{" "}
      ...
      {showReward && (
        <div
          className="mb-6 p-5 bg-amber-50 border border-amber-200 rounded-2xl text-center shadow-sm animate-fade-in"
          id="reward-banner"
        >
          ...
          <p className="text-xs text-slate-600 max-w-md mx-auto mt-1.5 font-sans font-medium leading-relaxed">
            You successfully simulated a complete lifestyle emission reduction
            week. Take these habits straight into your real carbon reduction
            journey!
          </p>
        </div>
      )}
