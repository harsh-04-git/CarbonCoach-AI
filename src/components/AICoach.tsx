...
export const AICoach: React.FC<AICoachProps> = ({ auditInput, profile, actions, committedIds }) => {
...
Your major carbon category is **${
        Object.entries(profile.breakdown).reduce((max, curr) => curr[1] > max[1] ? curr : max, ["", 0])[0].toUpperCase()
      }**.
...
Ask me any question about reducing utility bills, optimizing commute, drying clothes naturally, or switching ceiling fans dynamically instead of power-hungry ACs! What should we optimize today?`
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-4 animate-fade-in" id="ai-coach-root">
        ...
        <div className="md:col-span-4 bg-white rounded-3xl border border-emerald-100 p-5 shadow-sm flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 font-sans font-medium leading-relaxed mb-6">
              AI recommendations are cross-referenced with your custom audit. Ask the coach how to dry clothes naturally, substitute flights, or find easy, free carbon adjustments.
            </p>
          ...
          <div className="pt-6 border-t border-slate-100 text-center mt-6">
            <span className="inline-flex items-center gap-1.5 text-[10px] text-slate-400 font-semibold font-sans">
              Powered by <span className="font-bold text-emerald-600">Gemini 3.5 Flash</span> on Server
            </span>
          </div>
        </div>

        {/* Right column: Chat box */}
        <div className="md:col-span-8 bg-white rounded-3xl border border-emerald-100 shadow-sm flex flex-col h-[500px] overflow-hidden" id="chat-stage">
          {/* Chat header */}
          ...
          {/* Chat Messages */}
          <div className="flex-grow overflow-y-auto p-4 space-y-4" id="messages-container">
            {messages.map((m, idx) => {
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-emerald-50/50 border border-emerald-100 rounded-2xl rounded-tl-none p-4 flex items-center gap-2.5 text-xs text-emerald-800 font-mono font-bold uppercase tracking-wider">
                  <Loader2 className="w-4 h-4 text-emerald-600 animate-spin" /> Coach is analyzing profile...
                </div>
              </div>
            )}
          ...
          {/* Chat Form Input */}
          <form onSubmit={handleFormSubmit} className="p-3 bg-slate-50 border-t border-slate-100 flex gap-2">
            <input
              type="text"
              value={inputMessage}
              disabled={loading}
